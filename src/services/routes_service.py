...
class RouteService: #creates Route_Service class, used to find a route between origin lat and long pts on the map and destination lat and long pts on the map (Google)
    def __init__(self, api_key): #this is the definition that takes the api key that src.services.route_data took from src.services.api
        ...
        self.api_key = api_key #api key goes from src.services.api to src.services.route_data to here, not hardcoded for privacy
        self.url = "https://routes.googleapis.com/directions/v2:computeRoutes" #the link to the routes api

    def get_route(self, data): #the definition used to get the route, takes lat/long data, the headers, and the transportation type from src.services.route_data from user input
        ...
        return route_data #sends the route data to src.services.route_data so that it can use the data found through this class
